...
static void backtrack(int i)
{
    ...
    if (top >= 2) {
        ...
        // We check the cost of C, if it is smaller than the best we continue our tracking
        // else we restore (We don't have to go further because it will be always greater than the best we already found)
        if(C.cost <= bestCost){
            push(C);
            backtrack(i);
            pop(); // undo REDUCE
